...
// 
$(document).on("submit","#addpassid", function(){
  $.post("query/loginExepass.php", $(this).serialize(), function(data){
    // var thisId = $(this).data('id');
     if(data.res == "invalid2")
     {
       Swal.fire(
         'Invalid',
     ...
     else if(data.res == "success2")
     {
      //  $('body').fadeOut();
      //  window.location.href="home.php?page=danhsachexam";
      Swal.fire(
        'Thành công',
        'Kích hoạt mã bảo vể thành công',
        'success'
      )
      // setTimeout(function(){ 
        // refreshDiv();
...
if (result.value) {
  ...
  else
  {
    ...
    text: "Bạn đã trả lời xong câu hỏi?",
    icon: 'warning',
